...
async function answer(b: Body) {
  ...
  const smart = await smartAdvisor({ q, thread, shown: Array.isArray(b.shown) ? b.shown.filter((x) => typeof x === "string").slice(-12) : [], state: b.state && typeof b.state === "object" ? b.state : null, prev: typeof b.prev === "string" ? b.prev.slice(0, 300) : undefined, pid: typeof b.pid === "string" ? b.pid : undefined, space }, { name: settings.advisor.name, commerce: settings.site.commerce }).catch((e) => { console.error("[advisor]", e instanceof Error ? e.message : e); return null; });
