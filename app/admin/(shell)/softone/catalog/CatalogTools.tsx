  ...
  return (
    <div className="grid gap-3 rounded-2xl border border-eu-line bg-white p-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button type="button" disabled={pending} onClick={() => start(async () => { setMsg(null); const r = await runProjection(); const x = r.result; setMsg(r.ok && x ? `Προβολή στο κατάστημα: προϊόντα +${x.products.created}, ~${x.products.updated}, ${x.products.unchanged.toLocaleString("el-GR")} ίδια${x.products.deactivated ? `, ${x.products.deactivated} κρύφτηκαν` : ""}${x.products.skipped.noBrand ? ` · ${x.products.skipped.noBrand} χωρίς κατασκευαστή` : ""} · κατηγορίες +${x.categories.created}, ~${x.categories.updated} (${x.categories.visible} ορατές) · ${x.specs.toLocaleString("el-GR")} χαρακτηριστικά γράφτηκαν · φίλτρα +${x.facets.created}, ~${x.facets.updated}, −${x.facets.removed} · τιμές φίλτρων: ${x.facetValues.values.toLocaleString("el-GR")} σε ${x.facetValues.rewritten.toLocaleString("el-GR")} προϊόντα που άλλαξαν (${(r.ms / 1000).toFixed(1)} s)` : `Προβολή: ΣΦΑΛΜΑ — ${r.error}`); })} className={secondary}><Store className="size-4" aria-hidden /> Προβολή στο κατάστημα</button>
