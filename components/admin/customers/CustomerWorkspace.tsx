...
type Any = any;
const TABS = [["profile", "Προφίλ"], ["addresses", "Διευθύνσεις"], ["orders", "Παραγγελίες"], ["devices", "Συσκευές & εγγυήσεις"], ["service", "Service"], ["consents", "Συναινέσεις"], ["logins", "Συνδέσεις"], ["loyalty", "Πόντοι"], ["wishlist", "Αγαπημένα"], ["notes", "Σημειώσεις"], ["erp", "SoftOne"], ["gdpr", "GDPR"]] as const;
...
export function CustomerWorkspace({ customer, stores, perms, erpConfigured }: { customer: Any | null; stores: { id: string; name: string; city: string }[]; perms: { write: boolean; export: boolean; service: boolean }; erpConfigured: boolean }) {
  ...
  return (
    <div className="eu-container grid gap-4">
      ...
      {c && tab === "wishlist" && <Card title="Αγαπημένα (λίστες)">{c.wishlists.length ? c.wishlists.map((l: Any) => <div key={l.id} className="grid gap-1 text-[length:var(--fs-14)]"><div className="font-bold text-eu-ink">{l.name} <span className="text-eu-muted font-normal">· {l.items.length} προϊόντα · {l.visibility === "link" ? "κοινόχρηστη" : "ιδιωτική"}</span></div><ul className="m-0 p-0 list-none grid gap-0.5">{l.items.map((i: Any) => <li key={i.id} className="grid grid-cols-[minmax(0,1fr)_110px_120px] gap-2 border-t border-eu-line-2 py-1"><span className="font-mono truncate">{i.productId}{i.note ? <span className="text-eu-muted font-sans"> — {i.note}</span> : null}</span><span className="tabular-nums text-eu-ink-2">{i.priceAtAdd ? `${Number(i.priceAtAdd).toLocaleString("el-GR")} €` : "—"}</span><span className="text-eu-muted">{dt(i.addedAt)}</span></li>)}</ul></div>) : <p className="m-0 text-eu-muted">Καμία λίστα.</p>}</Card>}
