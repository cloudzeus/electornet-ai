...
interface Case { q: string; prev?: string; type?: RegExp; maxPrice?: number; brand?: RegExp; title?: RegExp; maxW?: number; none?: boolean; text?: RegExp }
const CASES: Case[] = [
...
async function main() {
  ...
  for (const [k, c] of CASES.entries()) {
    ...
    n++; const t0 = Date.now();
    const a = await smartAdvisor({ q: c.q, prev: c.prev }, ctx);
    ...
    console.log(`${fails.length ? "✗" : "✓"} ${String(k + 1).padStart(2)} ${(ms / 1000).toFixed(1)}s  ${c.q}${fails.length ? `\n      → ${[...new Set(fails)].join(" · ")}` : ""}`);
