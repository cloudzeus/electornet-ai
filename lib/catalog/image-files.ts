 ...
 */
export type ImageKind = "photo" | "banner";
export interface ParsedImageName { sourceFile: string; key: string; model: string; seq: number; ext: string; isNew: boolean; kind?: ImageKind }
...
/**
 * Banners χαρακτηριστικών (τα γραφικά με κείμενο που έμπαιναν μέσα στην περιγραφή του παλιού site):
 *   <κλειδί>_<μοντέλο>_<NNN>.<ext> → 0074108020680_WW11DB8B95GHU4_003.jpg — ίδιο κλειδί με τις φωτογραφίες, χωρίς το «_EURONICS».
 */
const BANNER = /^([^_]+)_(.+?)\s*[-_](\d{3})\s*\.+(jpe?g|png|webp)$/i; // «_013..jpg» υπάρχει
export function parseBannerName(sourceFile: string): ParsedImageName | null {
  const m = BANNER.exec(sourceFile.normalize("NFC"));
  return m ? { sourceFile, key: m[1].trim(), model: m[2].trim(), seq: Number(m[3]), isNew: false, ext: m[4].toLowerCase(), kind: "banner" } : null;
}
...
export function bunnyPaths(p: ParsedImageName) {
  const dir = `products/${slugify(p.key) || "x"}${p.kind === "banner" ? "/banners" : ""}`;
  const base = `${slugify(p.model).slice(0, 60).replace(/-+$/, "") || "image"}-${String(p.seq).padStart(2, "0")}`;
  return { main: `${dir}/${base}.webp` };
}
