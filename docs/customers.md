...
## Cart by email & wishlist alerts
- **Cart by email** (`lib/cart/email.ts`, `POST /api/cart/email`): at the customer's request only (transactional — no marketing consent needed). Signed-in → one click to the account email; guest → email field. Stores a `SavedCart` (lines, total, reason, source, evidence) with a token; the email (`cart-emailed`) carries `/kalathi?restore=<token>` which rebuilds the cart on any device (`CartRestore`, 30 days, `openedAt`/`restoredAt` tracked). Entry points: the cart page button (`EmailCartButton`) and the exit-intent dialog (reason + send). Rate limit 3 / hour per email; `CustomerEvent cart-emailed` for customers. `GET /api/account/me` is the light session probe used by these client components.
- **Wishlist alerts** (`lib/wishlist/notify.ts` → `runWishlistAlerts`): price drop / offer when the current price is below the price the item was saved at (or below the last notified price → one email per drop), and back-in-stock when availability flips to in-stock. Respects item switches, customer status and the consent ledger (`price-drop` / `back-in-stock` email refusals). Writes `lastNotifiedPrice/At`, `lastSeenAvailability`, `lastStockNotifiedAt` and a `wishlist-alert` event. Triggers: `GET /api/cron/wishlist-alerts` (Bearer `CRON_SECRET`), after price/stock sync, or the buttons on `/admin/reports/wishlist` (dry run / run now, audited).
