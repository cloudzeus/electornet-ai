...
| Ρυθμίσεις (`lib/cms/settings.ts`, `SettingsProvider`) | `getSettings()` / `useSettings()` | CMS singletons `site`, `motion`, `advisor`, `stickers` — κάθε τιμή/κείμενο/χρόνος κίνησης που δεν ανήκει σε προϊόν ή ζώνη | ISR 300s |
